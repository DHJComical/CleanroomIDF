    ...
    public ActionResult<ItemStack> onItemRightClick(World worldIn, EntityPlayer playerIn, EnumHand handIn) {
        ...
        if (isStackReady(playerIn, stack))
        {
            if (!worldIn.isRemote)
            {
                ...
                List<EntityPlayer> entities = worldIn.getEntitiesWithinAABB(EntityPlayer.class, IDLGeneral.ServerAABB(basePos.add(-range, -range, -range), basePos.add(range, range, range)));
