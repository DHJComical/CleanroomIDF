...
public class ItemSkillTauntNearbyToGiven extends ItemSkillBase {
    ...
    public boolean itemInteractionForEntity(ItemStack stack, EntityPlayer playerIn, EntityLivingBase target, EnumHand handIn) {
        if (isStackReady(playerIn, stack))
        {
            ...
            List<EntityLiving> entities = playerIn.world.getEntitiesWithinAABB(EntityLiving.class, IDLGeneral.ServerAABB(basePos.add(-getRange(stack), -getRange(stack), -getRange(stack)),
                    basePos.add(getRange(stack), getRange(stack), getRange(stack))));
