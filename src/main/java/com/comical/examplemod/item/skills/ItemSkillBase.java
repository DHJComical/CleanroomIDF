package com.comical.examplemod.item.skills;

import java.util.List;

import com.comical.examplemod.Main;
import com.comical.examplemod.init.ModCreativeTab;
import com.comical.examplemod.item.ItemBase;
import com.comical.examplemod.util.CommonDef;
import com.comical.examplemod.util.CommonFunctions;
import com.comical.examplemod.util.IDLSkillNBT;
import com.comical.examplemod.util.MessageDef;
...
public class ItemSkillBase extends ItemBase {
    ...
    public void SendDefaultMsg(EntityPlayer player, ItemStack stack, SKILL_MSG_TYPE msg_type)
    {
        ...
        {
            case CD:
                CommonFunctions.SendMsgToPlayer((EntityPlayerMP)player, stack.getTranslationKey()+ IDLSkillNBT.IN_CD_DESC_KEY);
            case SUCCESS:
                CommonFunctions.SendMsgToPlayer((EntityPlayerMP)player, stack.getTranslationKey()+ IDLSkillNBT.SUCCESS_DESC_KEY);
                default:
                    break;
        }
    }
    ...
    {
        CommonFunctions.SendMsgToPlayer(player, "examplemod.skill.msg.cool_down");
    }
    ...
    public void addInformation(ItemStack stack, World world, List<String> tooltip, ITooltipFlag flag) {
        ...
        {
            if (showCDDesc)
                tooltip.add(I18n.format("examplemod.skill.shared.cool_down_desc", getCoolDown(stack)));
            if (showDamageDesc && getVal(stack) > 0)
                tooltip.add(I18n.format("examplemod.skill.shared.power_desc", getVal(stack)));
            if (showRangeDesc)
                tooltip.add(I18n.format("examplemod.skill.shared.range_desc", getRange(stack)));
            if (showDuraDesc)
                tooltip.add(GetDuraDescString(getDura(stack)));

            if (maxLevel != 1)
            {
                tooltip.add(I18n.format("examplemod.skill.shared.level_desc", IDLSkillNBT.getLevel(stack), maxLevel));
            }
        }
    }
...
//    	}
        ...
        String strMaxLv = lv == maxLevel ? I18n.format("examplemod.skill.shared.lv_max") : "";

        return I18n.format("examplemod.skill.shared.name_format_with_lv",strMain, IDLSkillNBT.getLevel(stack), strMaxLv);
    }

    public String GetDuraDescString(float val)
    {
        return I18n.format("examplemod.skill.shared.duration_desc", val);
    }
