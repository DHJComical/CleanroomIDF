...
public class ItemSkillFireBlast extends ItemSkillBase {
    ...
    public ActionResult<ItemStack> onItemRightClick(World worldIn, EntityPlayer playerIn, EnumHand handIn) {
        ...
        {
            //Main.Log("Trigg'd:" + worldIn.getWorldTime());
            if (!worldIn.isRemote)
            {
                ItemSkillFireBlast skillThunderFall = ((ItemSkillFireBlast)stack.getItem());

                Vec3d basePos = playerIn.getPositionVector();
                List<EntityLiving> entities = worldIn.getEntitiesWithinAABB(EntityLiving.class, IDLGeneral.ServerAABB(basePos.add(-base_range, -base_range, -base_range), basePos.add(base_range, base_range, base_range)));
