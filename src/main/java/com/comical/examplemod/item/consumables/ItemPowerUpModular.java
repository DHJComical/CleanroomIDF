package com.comical.examplemod.item.consumables;

import java.util.List;

import com.comical.examplemod.entity.creatures.EntityModUnit;
import com.comical.examplemod.item.ItemBase;
import com.comical.examplemod.util.CommonFunctions;
import com.comical.examplemod.util.EntityUtil;
import com.comical.examplemod.util.NBTStrDef.IDLNBTUtil;
...
import static com.comical.examplemod.util.NBTStrDef.IDLNBTDef.MARK_TOTAL_COUNT;
    ...
    public boolean itemInteractionForEntity(ItemStack stack, EntityPlayer playerIn, EntityLivingBase target, EnumHand handIn) {
        if (target instanceof EntityModUnit)
        {
            ...
            if (legalTarget.is_mechanic)
            {
                ...
                {
                    CommonFunctions.SafeSendMsgToPlayer(playerIn, "examplemod.msg.upgrade_module.req_lv", String.valueOf(requireLv));
                    return false;
                }
                ...
                if (!playerIn.world.isRemote)
                {
                    ...
                    if (success)
                    {
                        ...
                        CommonFunctions.SafeSendMsgToPlayer(playerIn, "examplemod.msg.upgrade_module.ok", valueBefore, valueAfter);
