package com.comical.examplemod.item.weapon;

import java.util.List;

import com.comical.examplemod.item.ItemAdaptingBase;
...
public class ItemHealingGun extends ItemAdaptingBase {
    ...
    {
        //Main.Log("onCreatureStoppedUsing");
        Vec3d basePos = entityLiving.getPositionVector();
        float range = getRange(stack);
        List<EntityLivingBase> entities = worldIn.getEntitiesWithinAABB(EntityLivingBase.class, IDLGeneral.ServerAABB(basePos.add(-range, -range, -range), basePos.add(range, range, range)));
        //Main.Log("found %d targets", entities.size());
    ...
    void spawnParticles(EnumParticleTypes particleType, World worldIn, EntityLivingBase center)
    {
        ...
        for (float angle = 0f; angle < maxAngle; angle += deltaAngle)
        {
           // float radius = center.getRNG().nextFloat() * speedMagnitude;
            //Main.Log("spawn particle at %s, %s, %s", center.posX, center.posY, center.posZ);
