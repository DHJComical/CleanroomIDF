package com.comical.examplemod.item.goblet;
...
import static com.comical.examplemod.util.CommonDef.TICK_PER_SECOND;
...
public class ItemFlameGoblet extends ItemGobletBase {
    ...
    public void onUpdate(ItemStack stack, World worldIn, Entity entityIn, int itemSlot, boolean isSelected) {
        ...
        if (isSelected && !worldIn.isRemote)
        {
            ...
            if (level > 0 && worldIn.getWorldTime() % TICK_PER_SECOND == 0)
            {
                ...
                List<EntityLiving> entities = worldIn.getEntitiesWithinAABB(EntityLiving.class, IDLGeneral.ServerAABB(basePos.add(-level, -level, -level), basePos.add(level, level, level)));
    ...
    public void addInformation(ItemStack stack, World world, List<String> tooltip, ITooltipFlag flag) {
        String mainDesc = I18n.format(stack.getTranslationKey() + ".desc", GetLevelFromEXP(GetCacheEXP(stack)));
