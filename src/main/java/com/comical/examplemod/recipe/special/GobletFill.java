package com.comical.examplemod.recipe.special;

import javax.annotation.Nonnull;

import com.comical.examplemod.item.goblet.ItemP2WGoblet;
	...
	public ItemStack getCraftingResult(@Nonnull InventoryCrafting var1) {
		...
		for(int i = 0; i < var1.getSizeInventory(); i++) {
			...
			if(!stack.isEmpty()) {
				...
				else if(stack.getItem().getTranslationKey().equals(ItemBlock.getItemFromBlock(Blocks.GOLD_BLOCK).getTranslationKey()))
				{
					payingXP+=9;
				}
				//Main.Log(stack.getItem().getUnlocalizedName() + " vs " + ItemBlock.getItemFromBlock(Blocks.GOLD_BLOCK).getUnlocalizedName());
			}
		}
