package com.comical.examplemod.blocks.tileEntity.orbs;
...
public class TileEntityNullifyOrb extends TileEntityOrbBase implements ITickable {
	...
	public void onSpawn(LivingSpawnEvent.CheckSpawn event) {
		...
		if (aabb == null)
		{
			...
			Main.LogWarning("A Nullify Orb @%s is not working correctly. Removing it.", pos);
			invalidate();
			return;
		}
		...
		if(event.getResult() != Event.Result.DENY && event.getEntityLiving() instanceof IMob) {
			if (aabb.contains(new Vec3d(event.getX(), event.getY(), event.getZ())))
			{
				...
				//Main.Log("Stopped spawning:"+event.getEntityLiving().getName());
				return;
			}
		}
	}
	...
	static 
	{
		register("examplemod:nullify_orb_basic", TileEntityNullifyOrb.class);
	}
